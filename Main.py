...
import Pyglet_window  # Import Pyglet Window Class
...
layout = [[sg.T("Welcome to the Epidemic Day Epidemic Simulation", font=("Lucida Grande", 18))],
          [sg.Slider(range=(5, 30), orientation="h", default_value=14, key="population", size=(40, 15)),
           sg.T("Population (square length 5-30 (25-900 total))")],
          [sg.Slider(range=(1, 100), orientation="h", default_value=10, key="infection_rate", size=(40, 15)),
           sg.T("Infection rate (1% to 100%)")],
          [sg.Slider(range=(1, 100), orientation="h", default_value=60, key="recovery_rate", size=(40, 15)),
          ...
          [sg.Slider(range=(1, 100), orientation="h", default_value=5, key="fatality_rate", size=(40, 15)),
           sg.T("Fatality rate (1% - 100%)")],
          [sg.Checkbox("Should fatality rate decrease?", default=False, key="FRD"), sg.VerticalSeparator(pad=None),
           sg.T("Rate will decrease at"), sg.Input(size=(3, 1), key="FDP"), sg.T("percent per cycle.")],
          [sg.Button("Start Simulation"), sg.Button("Exit")]]

sg.theme("DarkGreen")
starting_window = sg.Window("Epidemic Simulation", layout)  # Start PySimpleGUI window

while True:  # Loop to check for events within the PySimpleGUI window
...
graph = Pyglet_window.Chart(entity_map, infection_rate, recovery_rate, recovery_rate_increase, fatality_rate,
